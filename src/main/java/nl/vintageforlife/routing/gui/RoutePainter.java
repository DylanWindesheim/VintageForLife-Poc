...
/** Tekent de route op de kaart: lijnen tussen stops, pijlen voor richting, gekleurde markers en namen. */
public class RoutePainter implements Painter<JXMapViewer> {

    private final Route route;

    public RoutePainter(Route route) { this.route = route; }
    ...
    public void paint(Graphics2D g, JXMapViewer map, int w, int h) {
        ...
        // Blauwe lijn tussen elke twee opeenvolgende stops
        ...
        // Oranje stippellijn terug naar het depot (sluit de rit)
        if (stops.size() > 1) {
        ...
        // Markers voor elke stop: groen voor het depot, rood voor de overige stops
        for (int i = 0; i < stops.size(); i++) {
            Point p = geoToScreen(stops.get(i).getAdres(), map, bounds);

            // Schaduw onder de marker
            ...
            // Naam van de klant naast de marker op een donker label
    ...
    /** Zet een geografische coördinaat om naar een pixelpositie op het scherm. */
    ...
    /** Tekent een kleine pijlpunt op het midden van een lijn om de rijrichting aan te geven. */
